...
/** Loads in the Trigger data from the "level#_triggers.cfg" config file.
 *
 * This config file doesn't contain paths to any other config files, so this
 * manager will load all of the trigger data (ID, coordinates, type) directly.
 */
...
{
	public:
		/* When creating the Trigger Manager, pass in the directory where all
		 * the config files will be found under:
		 *   "./media/world#/map#/"
		 */
		Trigger_Manager(const std::string &working_directory);
		virtual ~Trigger_Manager();

		std::string Get_WorkingDirectory();

		/* Once the Trigger Manager has been created, assign a single config
		 * file to load:
		 *   "level#_triggers.cfg"
		 *
		...
		/* Return the (loaded) triggerArray to the Level. */
		TriggerArray Get_TriggerArray();


		/* Virtual function: Actually extract the data from the config file */
		virtual bool Load_Settings_From_Config();

		/* Virtual function: Display all of data loaded into triggerArray */
		...
		/* Entry: "num_triggers" */
		bool Read_Num_Triggers(std::ifstream &fs);

		/* Entry: "triggers_config" */
		bool Read_Triggers_Config(std::ifstream &fs);
	...
	protected:
		...
		/* You can get numTriggers from triggerArray.size() */
